...
client.on('messageCreate', async (message) => {
  ...
  .setLabel(`Support`)
  .setStyle('LINK')
  .setURL(`https://discord.gg/arop`)
  )
  if (message.content === `<@${client.user.id}>`) { 
    return message.channel.send({ embeds: [new MessageEmbed().setColor(`DARK_BUT_NOT_BLACK`).setTitle(message.guild.name).setDescription(`Hey ${message.author},\nMy Prefix here is: \`${prefix}\`\nServer Id: \`${message.guild.id}\`\n\nType \`${prefix}\`**help** To Get The Command List.`).setFooter({text: `Developed with ❤️ By SCRENOX |`, iconURL: message.guild.iconURL({dynamic: true})})], components: [row] });
  }
...
if(dbs === true){
  ...
  .setDescription(`fuck off and don't disturb me`)
